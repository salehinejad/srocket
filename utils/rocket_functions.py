...
@njit("Tuple((float64[:],int32[:],float64[:],int32[:],int32[:],float64[:,:], float64[:,:]))(int64,int64)")
def generate_kernels(input_length, num_kernels):
    ...
    kernels_g = np.zeros((num_kernels,np.max(candidate_lengths)),dtype = np.float64) # N_kernels x length of kernel
    ...
    for i in range(num_kernels):
        ...
        # l1 norm of weights for l1 pruning
        weights_norm1[0,i] = np.sum(np.abs(weights[a1:b1]))
        weights_norm1[1,i] = np.sqrt(np.sum(weights[a1:b1]**2))

        # print(weights_norm1[0,i],i)

        kernels_g[i,0:_length] = _weights - _weights.mean()
    ...
    return weights, lengths, biases, dilations, paddings, kernels_g, weights_norm1
...
def apply_kernels(X, kernels):
    ...
    # graph_signal = np.zeros((num_examples,num_kernels,2))
    for i in prange(num_examples):
        ...
        for j in range(num_kernels):
            ...
            # graph_signal[i,j,0]=_X[i, a2]
            # graph_signal[i,j,1]=_X[i, a2+1]
